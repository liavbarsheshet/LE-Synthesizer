from modules.parser.sentence import Sentence
from modules.parser.grammar import *
from modules.parser.chart import *
...
class Parser:
    ...
    def __init__(self, grammar, sentence, debug=False):
        ...
        self.sentence = (
            sentence if isinstance(sentence, Sentence) else Sentence(sentence)
        )
        self.debug = debug
    ...
    def init_first_chart(self):
        ...
        row = ChartRow(
            Rule(Parser.GAMMA_SYMBOL, [self.grammar.start_symbol or "S"]), 0, 0
        )
